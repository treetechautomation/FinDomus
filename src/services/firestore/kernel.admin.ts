import { adminDb } from '../../lib/firebase-admin';
import { runFinancialKernel } from '@/core/finance/kernel';

export async function loadKernelContextAdmin(userId: string) {
  if (!userId) throw new Error("userId required");

  // Query 6 essential collections from adminDb
  const [
    transactionsSnap,
    liabilitiesSnap,
    recurringExpensesSnap,
    accountsSnap,
    investmentsSnap,
    wealthProfileSnap
  ] = await Promise.all([
    adminDb.collection('transactions').where('userId', '==', userId).get(),
    adminDb.collection('liabilities').where('userId', '==', userId).get(),
    adminDb.collection('recurring_expenses').where('userId', '==', userId).get(),
    adminDb.collection('accounts').where('userId', '==', userId).get(),
    adminDb.collection('investments').where('userId', '==', userId).get(),
    adminDb.collection('wealth_profiles').where('userId', '==', userId).limit(1).get()
  ]);

  const transactions = transactionsSnap.docs.map(d => ({ id: d.id, ...d.data() }));
  const liabilities = liabilitiesSnap.docs.map(d => ({ id: d.id, ...d.data() }));
  const recurringExpenses = recurringExpensesSnap.docs.map(d => ({ id: d.id, ...d.data() }));
  const accounts = accountsSnap.docs.map(d => ({ id: d.id, ...d.data() }));
  const investments = investmentsSnap.docs.map(d => ({ id: d.id, ...d.data() }));
  const wealthProfile = wealthProfileSnap.empty
    ? null
    : { id: wealthProfileSnap.docs[0].id, ...wealthProfileSnap.docs[0].data() };

  // Run the kernel to get consolidated results!
  const kernelResult = runFinancialKernel({
    accounts,
    investments,
    liabilities,
    transactions,
    recurringExpenses,
    taxObligations: [],
    wealthProfile,
    monthlyClosures: [],
    investmentAnalytics: null,
  });

  return {
    accounts,
    investments,
    liabilities,
    transactions,
    recurringExpenses,
    taxObligations: [],
    wealthProfile,
    monthlyClosures: [],
    investmentAnalytics: null,

    netWorth: kernelResult.financialCore.netWorth,
    cashBalance: kernelResult.financialCore.cashBalance,
    emergencyReserve: {
      reserveAmount: kernelResult.reserve.reserveAmount,
      essentialMonthlyExpenses: kernelResult.reserve.essentialMonthlyExpenses,
      targetMonths: kernelResult.reserve.targetMonths,
      targetAmount: kernelResult.reserve.targetAmount,
      reserveGap: kernelResult.reserve.reserveGap,
      reservePercent: kernelResult.reserve.reservePercent,
      coveredMonths: kernelResult.reserve.coveredMonths,
      liquidityAssets: kernelResult.reserve.liquidityAssets,
      excludedAssets: kernelResult.reserve.excludedAssets,
    },
    monthlyIncome: kernelResult.dre.receitaTotal || 0,
    monthlyExpenses: kernelResult.dre.despesasOperacionais || 0,
    selicRate: 10.75, // Default SELIC fallback
    dre: {
      taxaAcumulacao: kernelResult.dre.taxaAcumulacao || 0,
      essenciais: kernelResult.dre.essenciais || 0,
    },
  };
}
