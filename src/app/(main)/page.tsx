...
import { AlertTriangle, ArrowDown, ArrowUp, Banknote, Building2, Landmark, TrendingUp, Users, Sparkles, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const [showLegacyDetails, setShowLegacyDetails] = useState<boolean>(false);
  const { showFinancialValues } = useVisibility();
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      {/* Gaveta Colapsável do Legado Técnico */}
      <div className="pt-4 border-t border-zinc-900/80">
        <button
          onClick={() => setShowLegacyDetails(!showLegacyDetails)}
          className="w-full flex items-center justify-between p-4 rounded-2xl bg-zinc-950/40 border border-zinc-900 hover:bg-zinc-900/10 hover:border-zinc-800 transition-all duration-300 font-bold text-xs uppercase tracking-widest text-zinc-400 hover:text-white"
        >
          <span>📊 DETALHES PATRIMONIAIS & HISTÓRICO LEGADO</span>
          {showLegacyDetails ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
        </button>
      </div>
