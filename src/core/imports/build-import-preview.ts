...
export type ImportPreviewRow = {
  ...
  reasons: string[];
  suggestedTransferPairId?: string;
  suggestedTransferScore?: number;
  suggestedTransferConfidence?: string;
  suggestedTransferReason?: string;
};
...
export function buildImportPreview(transactions: any[]) {
  ...
  if (transferRows.length > 1) {
    ...
    for (const sourceRow of transferRows) {
      const sourceId = `tmp_${sourceRow.index}`;
      if (pairedIds.has(sourceId)) continue;

      const sourceCandidate = candidates.find((c) => c.id === sourceId)!;
      const matches = findBestMatches(sourceCandidate, candidates);
      const bestMatch = matches.find((m) => !pairedIds.has(m.targetId));

      if (bestMatch && bestMatch.score > 0) {
        pairedIds.add(sourceId);
        ...
        console.log(`  -> ${sourceRow.transaction.date} | R$ ${sourceRow.transaction.amount} | ${sourceRow.transaction.description}`);
        console.log(`  -> ${targetRow.transaction.date} | R$ ${targetRow.transaction.amount} | ${targetRow.transaction.description}`);
      }
    }
  }
  // -----------------------------------------
  ...
  return {
    rows,
    totals: {
      grossExpenses: Number(totals.expenses.toFixed(2)),
      estornos: Number(totals.income.toFixed(2)),
      transfer: Number(totals.transfer.toFixed(2)),
      netInvoice: Number((totals.expenses - totals.income).toFixed(2)),
