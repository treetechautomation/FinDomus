    ...
    dataLines.map(async (line) => {
      ...
      if (
        ...
        return enrichInstallment(
          {
            ...(await classifyTransaction(rawText, amount > 0 ? -Math.abs(amount) : amount)),
            date,
          },
          rawText
        );
