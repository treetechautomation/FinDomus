...
import { RevenueDashboard } from "@/components/pessoal/revenue-dashboard";
import { RevenueChartPanel } from "@/components/pessoal/revenue-chart-panel";
    ...
    async function loadPessoalData() {
      try {
        const currentMonthKey = getCurrentMonthKey();
          const [transactionsResult, budgetsResult, wealthResult, closureResult, openingResult] = await Promise.all([
            getPersonalTransactions(uid),
            getBudgets(currentMonthKey),
            getWealthProfile(uid),
            getMonthlyClosure("PF", currentMonthKey),
            getMonthOpening("PF", currentMonthKey),
          ]);
          setMonthOpening(openingResult);

        setMonthClosure(closureResult);
        setPersonalTransactions(transactionsResult || []);
    ...
    useEffect(() => {
      async function loadSelectedMonthClosure() {
          const [closureResult, openingResult] = await Promise.all([
            getMonthlyClosure("PF", selectedMonthKey),
...
.map((item: any) => {
  ...
  async function handleCloseMonth() {
    try {
      ...
      const result = await closeMonthlyCompetence("PF", selectedMonthKey);
  ...
  async function handleReopenMonth() {
    if (!monthClosure?.id) return;

    try {
      setClosingMonth(true);
      await reopenMonth(monthClosure.id);
      const result = await getMonthlyClosure("PF", selectedMonthKey);
      setMonthClosure(result);
    } finally {
      setClosingMonth(false);
    }
