...
const [isMarketOpen, setIsMarketOpen] = useState(false);
  ...
  async function loadMarket() {
    try {
      ...
      const now = new Date(); const hour = Number(now.toLocaleString('en-US', { hour: '2-digit', hour12: false, timeZone: 'America/Sao_Paulo' })); const weekday = now.toLocaleString('pt-BR', { weekday: 'short', timeZone: 'America/Sao_Paulo' }).toLowerCase(); const weekend = weekday.includes('sáb') || weekday.includes('dom'); const marketOpen = weekend ? false : hour >= 10 && hour < 18; setIsMarketOpen(marketOpen);
  }
  ...
  const marqueeItems = useMemo(() => {
    const fallback: MarketTickerItem[] = [
